...
<limitations-and-future-work>
The main limitation of the project is the limited impact of images. The pure mathematical nature of the plots and the complexity of the algorithms make it difficult to extract valueable insights from them, especially for business users. The search of techniques for better use of the images is surely a aspect to improve. A first idea is the integration with a more precise description of the images or perhaps raw data as support of the images itself. This is especially important for better identifying patterns and insights. This would need a deep study of how to extract the most relevant information from the images, if this is possible@mirage.\
An important improvement would be to enhance the efficiency as mentioned in @stage-conclusion, reducing the execution time. In the current version the pipeline has two different ways to request the #gls("large language model"): a LLM wise and an Algorithm-wise. But more sophisticated ways to adapt the technical difficulties of the infrastructure could be explored.\
