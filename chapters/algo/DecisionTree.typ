...
A multitude of splitting criteria exist; the following are the most common (for a more complete list, see @data_mining_with_decision_trees, @decomposition-knowledge-detection). These criteria are usually based on measures of impurity, which quantify how mixed the classes are in a node. The goal is to find splits that create child nodes that are purer than the parent node.
...
Where $p_i$ is the proportion of instances belonging to class $i$ in the node, and $K$ is the total number of classes.
...
$ upright("IG") = upright("Gini") \( upright("parent") \) - sum_(upright("child")) frac(\| D_(upright("child")) \|, \| D \|) upright("Gini") \( upright("child") \) $
...
$ upright("IG") = upright("Entropy") \( upright("parent") \) - sum_(upright("child")) frac(\| D_(upright("child")) \|, \| D \|) upright("Entropy") \( upright("child") \) $
...
On the other hand, as described in the official Scikit-learn documentation @scikit-docs, the time complexity can be optimized to $O \( n dot.op p dot.op log \( n \) \)$ thanks to clever tracking of the general order of indices of the features, avoiding sorting at each node. Feature-level parallelization can be achieved by evaluating the splits to additionally improve performance.\
...
An advantage of the decision tree structure is the natural handling of #gls("categorical_features") without the need for encoding.
...
$ upright("Importance")_j = frac(sum_("nodes with feature " j) upright("IG")_("node") times frac(n_("node"), n), "total reduction of impurity") $
...
For a single instance, it is possible to trace the complete path from root to leaf and list all the comparisons (feature \<= threshold) that led to the prediction. This is a #strong[huge advantage for explainability] compared to black-box models. Every prediction is completely traceable locally.\
```
...
Finally, for regression tasks, piecewise-constant predictions can make it difficult to understand the relationship between features and the target variable, especially when the tree is deep and captures complex interactions.\
