...
<soft-margin-svm>
In the realistic case where the data are #strong[not linearly separable], due to the presence of noise, outliers, or overlapping classes, we allow some points to violate the margin. This can be achieved by introducing #strong[slack variables] $xi_i gt.eq 0$ that measure how much a point violates the margin:
...
and then calculating the scalar product $phi.alt \( x_i \)^T phi.alt \( x_j \)$, using a kernel function that returns the same result directly from the original data:

$ K \( x_i \, x_j \) = phi.alt \( x_i \)^T phi.alt \( x_j \) $ without the need to explicitly calculate $phi.alt$.\

The most common kernels are @svm-kernels: 
...
Transforms the data into an infinite-dimensional space. It is the #strong[most commonly used kernel] thanks to its flexibility, which makes it suitable for describing complex patterns. It also has only a single hyperparameter $gamma$ ($gamma$) to tune, making it easy to use.\
This hyperparameter controls the influence of a single training example. The larger the value of $gamma$, the closer other examples must be to be affected, leading to a higher risk of overfitting. Conversely, a smaller value of $gamma$ means that even points far from the decision boundary can influence it. In this case, a value of $gamma$ that is too small can lead to underfitting, as the model may fail to capture the complexity of the data.

#strong[\4. Sigmoid Kernel]

$ K \( x_i \, x_j \) = tanh \( alpha x_i^T x_j + c \) $
The sigmoid kernel resembles the activation function of a neural network and can be used in datasets where the relationship between features is expected to be "threshold-like" @sigmoid-kernel-medium, split into hard decision regions. However, it is less commonly used than the RBF kernel and can be more difficult to tune, but it can sometimes be useful as an alternative to a full neural network.

=== Time complexity
<sub:time-complexity-svm>
The time complexity for SVM training is heavily dependent on the different variants of the algorithm and the size of the dataset. In general, the training time complexity is between $O \( n^2 p \)$ and $O \( n^3 p \)$, where $n$ is the number of training samples and $p$ is the number of features. This is because SVM training involves solving a quadratic optimization problem, which can be computationally expensive, especially for large datasets. To mitigate the computational bottleneck of kernel use in SVM, various optimizations can be employed. Approximate kernel methods, such as the #gls("nystrom method") or #gls("random fourier features"), can reduce the computational cost of kernel SVMs by approximating the kernel matrix using sampling or random Fourier transformations. Additionally, #gls("sequential minimal optimization") breaks down the optimization problem into smaller subproblems that can be solved analytically. 
Even with these efficient optimization algorithms, using sophisticated kernels leads to higher computational costs, which is the trade-off for better predictive performance.\
On the other hand, linear SVMs with proper optimization, like #gls("linear programming") or #gls("stochastic gradient descent"), lower their time complexity to $O \( n p \)$. This makes linear SVMs more scalable for large datasets, though they are limited to linear patterns.\
For *inference*, the time complexity is again related to the kernel function and the number of support vectors, which usually grows with the size of the training set. Generally, it is between $O \( m p \)$ and $O \( m n \)$, where $m$ is the number of support vectors, while for linear SVMs the inference time complexity is $O \( p \)$, as the prediction is a simple dot product between the input features and the coefficients of the hyperplane.
...
The model represents the solution as a #strong[linear combination of kernel products] of the form
...
The closer the point is to the hyperplane (i.e., $d(x)$ close to 0), the less confident the prediction is, while points far from the hyperplane (i.e., $d(x)$ with large value) are predicted with higher confidence.\

=== Explainability and interpretability metrics
<sub:metrics-for-interpretability-svm>
==== Platt Scaling
<sub:platt-scaling-svm>
Out of the box SVM does not produce a measure of the uncertainty of its predictions, as it outputs a hard classification (+1 or -1) rather than a probability. To obtain estimates of the probability $P \( y = 1 | x \)$, a common approach is to use #strong[Platt scaling], which fits a logistic regression model to the SVM decision values $f(x) = beta_0 + sum_(j=1)^p beta_j x_j$ (which are proportional to the signed distances from the hyperplane) on a validation set. The formula for Platt scaling is:
$ P \( y = 1 | x \) = frac(1, 1 + exp \( A f\(x\) + B \)) $

Where $A \, B$ are parameters learned on a validation set, calibrating the decision values to probabilities.
...
As discussed in @sub:internal-representation-svm, SVM as a model has very important limitations in interpretability. Linear SVM can be explained through the coefficients, which indicate the orientation of the decision boundary. \
