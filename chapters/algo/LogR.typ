...
The model internally represents the values as a vector of weights, $beta = \[ beta_0 \, beta_1 \, . . . \, beta_p \]$. The presence of #gls("categorical_features") is standardly resolved using one-hot encoding, just like in linear regression. \
...
+ #strong[Linearity of the log-odds:]
  ...
  Non-linear relationships remain uncaptured and must be added manually.

+ #strong[Complete separation:] if a feature perfectly separates the
  two classes, the model cannot identify a finite weight since the algorithm will tend to push the weight to $+ oo$ or $- oo$, diverging. Usually, the solution is to remove this separating feature, since it is likely just a proxy for the target variable.
...
+ #strong[Absence of multicollinearity:] highly correlated features cause coefficient instability. This is particularly relevant because iterative methods may oscillate and require very small learning rates ($alpha$).

Homoscedasticity is no longer a requirement because the response can only be 0 or 1.

==== Preprocessing
<sub:preprocessing-logr>
To verify the suitability of logistic regression in the classification task for a given dataset, several methods can be used. The @corr_matrix can be used to identify highly correlated features, which can lead to multicollinearity issues. If such features are found, they can be removed or combined to reduce redundancy and improve model stability. \
For identifying anomalies in the other assumptions, plot visualization can be used. See @sub:metrics-for-interpretability-logr for more details.
...
<sub:metrics-logr>
The following is a list of metrics for evaluating the predictive performance of logistic regression in the context of the classification task. For the general metrics see @cap:classification-metrics.\
...
A coefficient with a large absolute Z-value indicates that the coefficient is significantly different from zero, suggesting that the corresponding feature has a significant impact on the predicted probabilities. \ 
...
Similar to Linear Regression (@sub:feature-effect-lr), the feature effect plot represents the impact of a feature on the prediction. In the logistic regression case, the effect is not on the predicted value but on the predicted probability, which is more intuitive to understand for most users. \

For every feature value, calculate:

$ P \( y = 1 | x_j = v \, x_(upright("other")) = upright("mean") \) $
