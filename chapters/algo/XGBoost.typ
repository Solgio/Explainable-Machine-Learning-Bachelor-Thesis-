...
$ Omega \( f \) = gamma T + 1 / 2 lambda sum_(j=1)^T w_j^2 $ penalizing both the number of leaves ($T$) and the leaf weight magnitude ($w$) using $gamma$ and $lambda$ respectively.
...
Using a block structure, which moves sorting outside the tree construction without re-sorting at each iteration, the time complexity is reduced to:
...
In reality, $n dot.op f$ is often reduced to only data entries with non-missing values thanks to the sparsity-aware algorithm, resulting in a significant speedup for sparse datasets that can amount to 50 times @xgboost.

For *inference*, the algorithm needs to traverse $K$ trees sequentially, following the path from root to leaf for each tree. The time complexity is therefore $ O \( K dot.op d \) $.\
...
Nonetheless, XGBoost remains a very powerful algorithm, especially if paired with careful hyperparameter tuning, for example using Optuna (@sec:optuna).
...
Where $ upright("Gain")_j^(\( k \)) $ is the total loss reduction when feature $j$ splits tree $k$, averaged across all $K$ trees.
A feature with a higher gain is considered more important, but it is important to note that gain measurements could be biased towards features with high cardinality. \
...
$ upright("Weight")_j = 1 / K sum_(k = 1)^K upright("# splits on feature ") j $
