...
=== Expert Personas
<sub:expert-personas>
The expert personas techniques consists in the design of prompts that describes the role that the #gls("large language model") should play in the task. This technique should guide the model in generating a more precize, coherent and relevant output.\
Though the expert personas seem to be linked to a loss of accuracy in multiple benchmarks in the recent studys by Zizhao Hu @expert_personas_zizhaohu, the primary use of the #gls("large language model", plural:true) in this project is results evaluation and text generation. The data and metrics do not have to be generated and have only to be read directly from the prompt. The paper specifically talks about _"For tasks that depend on pretrained knowledge retrieval accuracy"_ @expert_personas_zizhaohu. It's reasonable to think that in such a context, loss of accuracy is not a critical issue. In the same paper, an alignment of behavior and style to the described personas is observed. This drift in behavior is critical in the task of describing the results of an algorithm in a human-readable way, as the style of the explanation is fundamental to make it accessible to non-expert stakeholders. 

=== Familiar formats
<sub:familiar-formats>
The extensive use of #gls("large language model", plural: true) has led to the emergence of some familiar formats that are widely used in the design of prompts. \
The principal features that these formats present, and that makes them particularly effective, are the following:
  ...
  Of the many formats available, some have emerged for their accuracy and efficiency. The most valuable one is *Markdown*, which has a clear and recognizable structure that allows the model to easily identify the different components of the prompt and their relationships. The use of markdown also allows to create a clear and organized structure for the generated explanations, making them more readable and accessible to non-expert stakeholders.\
