...
=== Scikit-learn
Scikit-learn provides implementations of most of the algorithms under consideration without the need to implement them from scratch, allowing the developer to focus on the analysis and explainability aspects of the project. \ Moreover, it provides a variety of metrics for evaluating the performance of the models, which is crucial for the analysis.\
...
=== Streamlit
Streamlit was chosen for the implementation of the #gls("gui") of the system. It enables quick and easy creation of interactive web applications for data analysis and machine learning, which is essential for providing a user-friendly interface for the system.
...
#side_by_side(
    ...
    ,[
        The design of the entire pipeline was made with extensibility in mind, providing the developer with the capability to easily add new algorithms, datasets, analysis techniques, and #gls("large language model", plural: true). \
...
+ #strong[Algorithm]: implements the machine learning algorithm and provides the necessary functionalities for training, prediction and evaluation. All the models are created using a factory pattern, enabling the easy addition of new algorithms without modifying the existing code.
...
To achieve the best possible design for the system, object-oriented programming principles and design patterns were applied. First, the SOLID principles @design-patterns-martin, @clean-code were applied. In particular:
...
==== Facade
The facade pattern was applied to the `orchestrator`, providing a single entry point `run_pipeline()` for algorithm initialization, result aggregation, and management of the entire process of generating prompts, sending requests to the #gls("large language model"), and collecting responses, hiding the complexity of the underlying implementation from the rest of the system.\

== Code implementation
<sec:code-implementation>
Consequently, the resulting product of the design process is structured in a modular and layered way, with clear interfaces and separation of concerns among the different components. The implementation of the system follows the design principles and patterns described in the previous sections, resulting in a codebase that is *extensible*, *maintainable*, and *scalable*. \
...
#let algorithm-steps = (
    (title:"NewAlgo.py", desc:"Describe the new model following \"baseMLAlgo\" interface and its template method using the abstract classes as guidelines."),
...
#let dataset-steps = (
    ...
    (title:"dataValidator.py", desc:"If a validation rule is being added, create a new class that implements the \"DataValidator\" interface, providing the necessary methods for validating the new dataset."),
...
#let shap-steps = (
    ...
    (title:"strategy.py", desc:"Define in the \"strategy\" which algorithms can use the new SHAP technique, using the algorithm registry for type safety."),
    (title:"plot_renderer.py", desc:"Add any new methods for rendering the SHAP values.")
)
